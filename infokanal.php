...
$coachId = (int)(getCurrentCoachId() ?? 0);
$pdo = getDB();
...
if ($selectedArticleId > 0) {
    ...
    if ($visible) {
        $pdo->prepare(
            'INSERT INTO info_article_reads_coach (article_id, coach_id, read_at) VALUES (?, ?, NOW())
             ON DUPLICATE KEY UPDATE read_at = VALUES(read_at)'
        )->execute([(int)$visible['id'], $coachId]);
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
    ...
    <span class="text-muted small">Tipy, novinky a důležité informace pro trenéry</span>
</div>
...
<ul class="nav nav-pills mb-3 flex-wrap gap-2">
    ...
    <li class="nav-item">
        <a class="nav-link <?= $selectedCategoryId === $cid ? 'active' : '' ?>"
           href="<?= BASE_URL ?>/infokanal.php?category=<?= $cid ?>">
            <?= h($category['name']) ?>
    ...
    <div class="col-lg-5">
        <div class="card shadow-sm border-0">
            ...
            <div class="list-group list-group-flush">
                ...
                ?>
                <a href="<?= BASE_URL ?>/infokanal.php?category=<?= (int)$article['category_id'] ?>&article=<?= (int)$article['id'] ?>"
