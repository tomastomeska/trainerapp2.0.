...
function initCustomSelect(nativeSel) {
    ...
    function renderOpts(q) {
        ...
        opts.forEach(function(opt) {
            ...
            item.addEventListener('mouseenter', function() { if (!selected) item.style.background = '#f8f9fa'; });
            item.addEventListener('mouseleave', function() { if (!selected) item.style.background = ''; });
    ...
    function updateTrigger() {
        var sel = null;
        opts.forEach(function(o) { if (String(o.value) === String(nativeSel.value) && o.value) sel = o; });
        if (sel) {
...
function updateRemoveButtons() {
    ...
    btns.forEach(function(b) { b.style.display = btns.length > 1 ? '' : 'none'; });
}
